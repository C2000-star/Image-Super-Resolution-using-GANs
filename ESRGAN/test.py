...
# added this line imported evaluate
...
def main(_argv):
    ...
    if FLAGS.down_up:
        ...
        sr_down = imresize_np(sr_img, 1/(FLAGS.scale)).astype(np.uint8)
        hr_down = imresize_np(hr_img, 1/(FLAGS.scale)).astype(np.uint8)
        # sr_size = sr_img.size
        # print(sr_size)
        # added this line with LPIPS new field
        str_format = "[{}] PSNR/SSIM/LPIPS: SR={:.2f}db/{:.2f}/{:.2f}"
        print(str_format.format(
            ...
            # added this line 
            evaluate_lpips(sr_img, hr_img))) 
        result_img_path = './output/' + os.path.basename(FLAGS.model) + '/Bic_SR_HR_' + os.path.basename(FLAGS.img_path)
        str_format = "[{}] PSNR/SSIM/LPIPS: LR={:.2f}db/{:.2f}/{:.2f}"
        print(str_format.format(
            os.path.basename(FLAGS.img_path),
            calculate_psnr(rgb2ycbcr(sr_down), rgb2ycbcr(hr_down)),
            calculate_ssim(rgb2ycbcr(sr_down), rgb2ycbcr(hr_down)),
            # added this line 
            evaluate_lpips(sr_down, hr_down))) 
        print("Percentage improvement")
        improvement = calculate_psnr(rgb2ycbcr(sr_img), rgb2ycbcr(hr_img))/calculate_psnr(rgb2ycbcr(sr_down), rgb2ycbcr(hr_down))
